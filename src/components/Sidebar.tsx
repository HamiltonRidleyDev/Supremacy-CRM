...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/re-engagement", label: "Re-engagement", icon: "heart" },
  ...
  },
  {
    label: "Admin",
    items: [
      { href: "/settings/engagement", label: "Scoring Config", icon: "sliders" },
...
function QuickStartToggle() {
  ...
  useEffect(() => {
    // Default is ON (mobile auto-shows Quick Start unless opted out)
    const val = localStorage.getItem(QUICK_START_LS_KEY) !== "false";
    setEnabled(val);
    setLoaded(true);
  }, []);
  ...
  // Only show toggle on mobile — Quick Start is a mobile-only feature
  if (!loaded || !isReady || !isMobile) return null;
